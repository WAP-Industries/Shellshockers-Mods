    ...
    get response(){
        if (this.isScript){
            ...
            return code.replace(variables.scene + '.render()', `
                    window['${onUpdateFuncName}'](${variables.babylon},${variables.players}); 
...
window[onUpdateFuncName] = function(BABYLON, players){
    try{
        for (const player of players){
            if (!player) return

            player.actor.removeHat()
            player.actor.removeStamp()

            const mesh = function(){
                const material = function(){
                    const m = new BABYLON.StandardMaterial("", player.actor.scene)
                    m.emissiveColor = new BABYLON.Color3.White()
                    m.diffuseTexture = new BABYLON.Texture('', player.actor.scene)
